...
-- Selecionar o nome do paciente e a data da receita a partir do ID do paciente na tabela Receitas:
SELECT nome, data FROM Receitas INNER JOIN Pacientes ON Receitas.paciente_id = Pacientes.id;
...
-- Selecionar o nome do médico, data do exame e resultados a partir do ID do médico na tabela Exames:
SELECT nome, data, resultados FROM Exames INNER JOIN Medicos ON Exames.medico_id = Medicos.id;
-- Selecionar o nome do paciente, especialidade do médico e horário da consulta a partir do ID do paciente e do médico na tabela Consultas:
SELECT p.nome, especialidade, data_hora FROM Consultas c
